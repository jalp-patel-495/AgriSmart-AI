...
class PredictionResponse(BaseModel):
    ...
    crop_confidence: Optional[float] = Field(None, description="Stage A dedicated crop classifier confidence [0.0, 1.0]")
    disease_confidence: Optional[float] = Field(None, description="Stage B crop-gated disease classifier confidence [0.0, 1.0]")
    quality_ok: Optional[bool] = Field(True, description="Whether leaf quality gate passed")
    is_supported: Optional[bool] = Field(True, description="Whether detected crop is one of 14 supported species")
    is_ood: Optional[bool] = Field(False, description="Whether input is out-of-distribution/unknown")
    ood_score: Optional[float] = Field(0.0, description="Calibrated OOD anomaly score")
    ood_status: Optional[str] = Field("in_distribution", description="OOD status description")
